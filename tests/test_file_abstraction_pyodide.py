...
# Always surfaced in pytest's "warnings summary" (regardless of -v/-s/-rs), so
# CI logs make it obvious whether these tests actually ran or were skipped.
if _node_env_available():
    warnings.warn(
        "pyodide _zarrFile tests: ENABLED (node + tests/js/node_modules found) "
        "-- running for real against a pyodide runtime.",
        stacklevel=1,
    )
else:
    warnings.warn(
        "pyodide _zarrFile tests: SKIPPED -- Node.js and/or `npm install` under "
        "tests/js not found (see tests/README.md).",
        stacklevel=1,
    )
